...
- 掃描並連線 myAir S1；短暫收集附近候選裝置，單台自動連線，多台則由使用者選擇。
- 執行單次量測。
...
- 唯讀同步裝置端歷史量測；資料確認寫入前後都不送出清除命令。
- 可在「連線」頁更換或忘記上次使用的裝置；多裝置清單顯示名稱、訊號強度與上次使用標記。
- 一次性（5／15／30／自訂分鐘）、週期性（15／30／60／120／240 分鐘）及每日固定時間排程。
- 手動與排程量測共用 FIFO 任務佇列，顯示目前任務與等待數，避免量測重疊。
- 排程保存在 SQLite；服務或手機重啟後恢復，斷線超過 10 分鐘寬限則記錄未執行而不累積補做。
