...
掃描在找到第一個相容候選後再收集 3 秒，以位址去重。只有一台候選時直接連線；多台時交由 UI 顯示名稱、RSSI 與上次使用標記，使用者選定後才保存偏好裝置。更換裝置期間暫停既有偏好裝置的背景重連，忘記裝置則清除偏好並停止背景連線。
...
重試之間設有 1.5 秒 quiet/settle window；若仍收到前次 ACK 或歷史通知，等待時間會重新起算。每次命令只接受一個 ACK，資料封包必須從序號 0 連續遞增，否則捨棄整批。協定封包沒有 attempt ID，因此此機制只能降低延遲 ACK 或舊封包混入新嘗試的風險，不能從 wire protocol 絕對證明封包世代。
